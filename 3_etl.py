import configparser
import psycopg2
from sql_queries import copy_table_queries, insert_table_queries


def load_staging_tables(cur, conn):
    """
    Loads data into stage tables from S3 source provided in config file
    :param cur: Cursor object to execute query.
    :param conn: Connection object to commit the query.
    
    :returns: Nothing. Prints to console as each query is executed.
    """
    for query in copy_table_queries:
        cur.execute(query)
        print("Executed: {}".format(query))
        conn.commit()


def insert_tables(cur, conn):
    """
    Transforms staging tables into schema provided in insert_tables_queries of sql_queries.py
    :param cur: Cursor object to execute query.
    :param conn: Connection object to commit the query.
    
    :returns: Nothing. Prints to console as each query is executed.
    """
    for query in insert_table_queries:
        print("Executing: {}".format(query))
        cur.execute(query)
        conn.commit()


def main():
    config = configparser.ConfigParser()
    config.read('dwh.cfg')

    conn = psycopg2.connect("host={} dbname={} user={} password={} port={}".format(*config['CLUSTER'].values()))
    cur = conn.cursor()
    
    load_staging_tables(cur, conn)
    insert_tables(cur, conn)

    conn.close()


if __name__ == "__main__":
    main()